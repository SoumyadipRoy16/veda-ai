...
export async function handleUpdateAssignment(request: Request, response: Response) {
	const questionTypeCatalog = await listQuestionTypes();
	const updated = await updateAssignment(String(request.params.assignmentId), {
		draft: request.body.draft,
		questionTypeCatalog,
		subject: request.body.subject,
		className: request.body.className,
...
export async function handleDeleteAssignment(request: Request, response: Response) {
	try {
		await deleteAssignment(String(request.params.assignmentId));
...
export async function handleConfirmAssignment(request: Request, response: Response) {
	try {
		const { assignment, generatedPaper } = await confirmAssignmentGeneration(String(request.params.assignmentId));
...
export async function handleGetAssignment(request: Request, response: Response) {
	const assignment = await getAssignmentById(String(request.params.assignmentId));
	if (!assignment) {
...
export async function handleGetGeneratedPaper(request: Request, response: Response) {
	const paper = await getGeneratedPaperByAssignmentId(String(request.params.assignmentId));
	if (!paper) {
...
export async function handleRegenerateAssignment(request: Request, response: Response) {
	try {
		const paper = await regenerateAssignment(String(request.params.assignmentId));
		response.json({ paper });
	} catch (error) {
